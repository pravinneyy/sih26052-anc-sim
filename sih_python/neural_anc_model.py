...
class AcousticSceneClassifier(nn.Module):
    ...
    def _init_weights(self):
        ...
        if self.use_hidden:
            ...
            with torch.no_grad():
                # Project domain knowledge matrix W through fc1 weight pseudo-inverse
                w_feat = W[:, :N_FEATURES]
                w_proj = w_feat @ torch.pinverse(self.fc1.weight)
                self.fc2.weight.copy_(w_proj)
                self.fc2.bias.copy_(W[:, -1])
        else:
            with torch.no_grad():
