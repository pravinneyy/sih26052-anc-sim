...
def synth_voice(n: int, sr: int, rng: np.random.Generator) -> np.ndarray:
    ...
    f0 = float(rng.uniform(105.0, 210.0))
    ...
    f1, f2, f3 = float(rng.uniform(450.0, 750.0)), float(rng.uniform(1300.0, 2100.0)), float(rng.uniform(2300.0, 3100.0))
    for h in range(1, 20):
        freq = f0 * h * vibrato
        formant = (
            np.exp(-((freq - f1) / 250.0) ** 2)
            + 0.65 * np.exp(-((freq - f2) / 350.0) ** 2)
            + 0.35 * np.exp(-((freq - f3) / 450.0) ** 2)
        )
...
def train(args) -> Path:
    ...
    scaler = torch.amp.GradScaler(device.type if device.type == "cuda" else "cpu", enabled=args.amp and device.type == "cuda")
        ...
        for batch in loader:
            ...
            with torch.amp.autocast(device_type=device.type, enabled=args.amp and device.type == "cuda"):
                ...
                if getattr(args, "freq_weighted_loss", False):
                    freq_w = torch.ones_like(target_mask)
                    freq_w[:, :, :128] = 1.5
                    freq_w[:, :, 128:] = 0.75
                    mask_loss = F.smooth_l1_loss(pred["mask"] * freq_w, target_mask * freq_w)
                else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--freq-weighted-loss", action="store_true", help="Apply 1.5x weighting to low speech frequencies (<4kHz).")
    return p
