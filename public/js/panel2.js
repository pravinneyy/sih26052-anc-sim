...
function runP2() {
  ...
  if (times.length > 0) {
    ...
    const baselineD = meanDbWindow(res.D.sm, Math.round(2.4 * FS), Math.round(3.4 * FS));
    const recD = recoveryMs(res.D.sm, fromIdx, baselineD, 1.2, Math.round(0.05 * FS));
    const recDEl = document.getElementById('r_recD');
    if (recDEl) recDEl.innerHTML = Math.round(Math.min(recD, 12)) + '<span class="u">ms</span>';
  } else {
    setVal('r_loss', 0.0, 1);
