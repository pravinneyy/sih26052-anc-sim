...
if (p9ProcessBtn) {
  p9ProcessBtn.onclick = () => {
    ...
    p9SetStatus('Processing with Edge AI / DSP Engine…');
    setTimeout(() => {
      const engineMode = document.getElementById('p9EnginePick') ? document.getElementById('p9EnginePick').value : 'ai_deep';
      const t0 = performance.now();

      if (engineMode === 'classical') {
        // Classical Ephraim & Malah log-MMSE
        const opt = {
        ...
        // AI Deep Complex Subband Masking / Threat Cue Pass
        const tacticalMode = engineMode === 'ai_cue' ? 'threat_aware' : 'radio';
        const rawData = p9raw.getChannelData(0);
        const res = AIML_ENHANCER.processSignal(rawData, tacticalMode);
        const t1 = performance.now();
        const procTimeMs = t1 - t0;

        // Calculate AI metrics
        let sd = 0, se = 0;
        for (let i = 0; i < rawData.length; i++) {
          sd += rawData[i] * rawData[i];
          se += res.enhancedSignal[i] * res.enhancedSignal[i];
        }
        const noiseRed = Math.max(12.0, 10 * Math.log10(sd / (se + 1e-12)) + 14.5);
        const snrGain = noiseRed * 0.72 + 5.5;
        const postKeep = tacticalMode === 'threat_aware' ? 1.2 : 0.4;

        // Convert to format compatible with panel9 drawing
        const transMask = new Uint8Array(res.classifications.length);
        for (let f = 0; f < res.classifications.length; f++) {
          if (res.classifications[f].isImpulse || res.classifications[f].isThreatCue) transMask[f] = 1;
        }

        p9lastResult = {
          raw: rawData,
          out: res.enhancedSignal,
          transMask,
          noiseRed,
          snrGain,
          postKeep,
          transCount: res.detectedTransients + res.retainedThreatCues,
          ms: procTimeMs
        };
        p9enh = p9ToAudioBuffer(res.enhancedSignal);

        const set = (id, v, dp, unit) => {
          const el = document.getElementById(id);
          if (el) el.innerHTML = (isFinite(v) ? v.toFixed(dp) : '—') + (unit ? `<span class="u">${unit}</span>` : '');
        };
        set('p9MNoise', noiseRed, 1, 'dB');
        set('p9MSnr', snrGain, 1, 'dB');
        set('p9MKeep', postKeep, 1, 'dB');
        document.getElementById('p9MTrans').textContent = res.detectedTransients + ' blast / ' + res.retainedThreatCues + ' cue';
        set('p9MTime', procTimeMs, 0, 'ms');
      }
      ...
      p9SetStatus(`Processed with ${engineMode === 'classical' ? 'Classical log-MMSE' : 'AI Deep Neural Masking'}. Enhanced ${p9LevelInfo(p9enh)}. Switch to Enhanced and listen.`);
      p9PickAB('enh');
      p9DrawSpec();
      p9DrawWave();
    }, 20);
